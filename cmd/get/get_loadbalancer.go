...
func PrintGetLbResult(resp *http.Response, o api.RESTOptions) {
	...
	for _, lbrule := range lbresp.LbRules {
		...
		if o.PrintOption == "wide" {
			...
			// Get catalog name, default to "default" if empty
			catalog := lbrule.Service.LLMType
			if catalog == "" {
				catalog = "default"
			}

			// Get trace_type, default to "-" if empty
			traceType := lbrule.Service.TraceType
			if traceType == "" {
				traceType = "-"
			}

			// Get backend protocol, only meaningful for fullproxy mode (mode 4)
			backendProto := "-"
			if lbrule.Service.Mode == 4 {
				backendProto = lbrule.Service.BackendProtocol
				if backendProto == "" {
					backendProto = "http1"
				}
			}
			...
			if lbrule.Service.Monitor {
				for i, eps := range lbrule.Endpoints {
					if i == 0 {
						// Get path match mode display string
						pathMatchMode := lbrule.Service.PathMatchMode
						if pathMatchMode == "" {
							pathMatchMode = "disabled"
						}
						// Get session header name
						sessionHdr := lbrule.Service.SessionHeaderName
						if sessionHdr == "" {
							sessionHdr = "-"
						}
						// Get CHWBL fields (shown for sel=8 and sel=10)
						chwblLvl := "-"
						chwblFlags := "-"
						chwblLF := "-"
						chwblRepl := "-"
						chwblSalt := "-"
						if lbrule.Service.Sel == 8 || lbrule.Service.Sel == 10 {
							if lbrule.Service.ChwblPrefixHashLevel > 0 {
								chwblLvl = fmt.Sprintf("%d", lbrule.Service.ChwblPrefixHashLevel)
							}
							if lbrule.Service.ChwblPrefixHashFlags > 0 {
								chwblFlags = fmt.Sprintf("0x%02x", lbrule.Service.ChwblPrefixHashFlags)
							}
							if lbrule.Service.ChwblMeanLoadFactor > 0 {
								chwblLF = fmt.Sprintf("%d", lbrule.Service.ChwblMeanLoadFactor)
							}
							if lbrule.Service.ChwblReplication > 0 {
								chwblRepl = fmt.Sprintf("%d", lbrule.Service.ChwblReplication)
							}
							if lbrule.Service.ChwblEnableCacheSalt {
								chwblSalt = "true"
							} else {
								chwblSalt = "false"
							}
			...
			if lbrule.Service.PortMax == 0 {
				data = append(data, []string{lbrule.Service.ExternalIP, fmt.Sprintf("%d", lbrule.Service.Port), protocolStr, lbrule.Service.Name, fmt.Sprintf("%d", lbrule.Service.Block), NumToSelect(int(lbrule.Service.Sel)), NumToMode(int(lbrule.Service.Mode), lbrule.Service.PpV2, lbrule.Service.Egress), catalog, backendProto, fmt.Sprintf("%d", len(lbrule.Endpoints)), fmt.Sprintf("%v", lbrule.Service.Timeout), BoolToMon(lbrule.Service.Monitor)})
			} else {
				data = append(data, []string{lbrule.Service.ExternalIP, fmt.Sprintf("%d-%d", lbrule.Service.Port, lbrule.Service.PortMax), protocolStr, lbrule.Service.Name, fmt.Sprintf("%d", lbrule.Service.Block), NumToSelect(int(lbrule.Service.Sel)), NumToMode(int(lbrule.Service.Mode), lbrule.Service.PpV2, lbrule.Service.Egress), catalog, backendProto, fmt.Sprintf("%d", len(lbrule.Endpoints)), fmt.Sprintf("%v", lbrule.Service.Timeout), BoolToMon(lbrule.Service.Monitor)})
			}
		}
	}
