...
func NewDeleteLoadBalancerCmd(restOptions *api.RESTOptions) *cobra.Command {
	...
	var deleteLbCmd = &cobra.Command{
		Use:   "lb <EXTERNAL-IP> [--tcp portNumber] [--udp portNumber] [--sctp portNumber] [--icmp portNumber] [--bgp] [--mark=<val>] [--name=<service-name>] [--host=<url>] [--path-prefix=<path>] [--path-match-mode=<mode>]",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			// Default PathMatchMode to "disabled" for backward compatibility
			// Always send path_match_mode to API so the handler can apply proper matching logic
			// The API handler will retry with empty PathMatchMode if needed for legacy rules
			if PathMatchMode == "" {
	...
	deleteLbCmd.Flags().StringVarP(&PathPrefix, "path-prefix", "", PathPrefix, "URL path prefix for L7 load balancing")
	deleteLbCmd.Flags().StringVarP(&PathMatchMode, "path-match-mode", "", PathMatchMode, "Path matching mode: disabled, prefix, or exact")
